...
class PrecontrattualeService implements ApplicationService
{
 ...
 /**
    ...
    public static function getDocumentoBozzaTitulus($pre){
        ...
        $doc->addRepertorio('CONTRCONV','Contratti e convenzioni');

        //TODO epigrafe insegnamento
        $doc->oggetto = 'A.A. ' . $pre->insegnmento->aa . '/' . (int) $pre->insegnmento->aa + 1 . ' - Contratto di insegnamento '.$pre->insegnamento->insegnamento; //almeno 30 caratteri
        $doc->addClassifCod('07/16');
        $doc->addAllegato('0 - nessun allegato');
        $doc->addVoceIndice('Istanza Online Contratti Personale non strutturato');

        //aggiunge come RPA l'Ufficio Contratti e Supplenze
        TitulusHelper::addRPA_Titulus($doc, '000066');

        //aggiunge in CC il Nucleo Didattico
        $unitaorganizzativa_uo = $pre->insegnamento->dip_cod;
        TitulusHelper::addCC_Titulus($doc, $unitaorganizzativa_uo);
        //aggiunge in CC il Servizio Personale Docente e di Ricerca
        TitulusHelper::addCC_Titulus($doc, '092130');
        //aggiunge in CC l'Ufficio Compensi
        TitulusHelper::addCC_Titulus($doc, '000073');
        ...
        TitulusExtraDoc::addIstituzione($registro, [
            'cod' => '170001',
            'denominazione' => 'Università Politecnica delle Marche',
